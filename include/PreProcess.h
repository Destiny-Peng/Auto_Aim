...
public:
	vector<LightBar> Contours2LightBar(vector<vector<Point>> contours) {		
		...
		for (vector<Point> contour : contours)rects.push_back(minAreaRect(contour));
		vector<LightBar> lightbars;
		for (int i = 0; i < rects.size();i++)lightbars.push_back(LightBar(rects[i]));
		return lightbars;
	}
	...
	vector<Vec3f>  Contours2Ellipse(vector<vector<Point>> contours, double dp = 1,double minDist = 50,double param1 = 100,double param2 = 47,int minRadius = 0,int maxRadius = 100 ){
		...
		for (int i = 0; i < contours.size(); i++) drawContours(img, contours, i, Scalar(0, 255, 0), 2);
	...
	vector<Armor> ArmorFliter(vector<Armor> putin){
		...
		for(int i = 0;i<putin.size();i++){
			if(putin[i].get_number())returnarmors.push_back(putin[i]);
		}
		return returnarmors;
	}
			...
			for (size_t i = 0; i < lightBars.size() - 1; i++) {
				for (size_t j = i + 1; j < lightBars.size(); j++) {
					if (lightBars[0].is_an_armor(lightBars[i], lightBars[j])) {
						...
						if (lightBars[i].Armor_index&&(armor.Score()>=armors[lightBars[i].Armor_index-1].Score()))armors[lightBars[i].Armor_index-1] = armor;
						else if (lightBars[j].Armor_index&&(armor.Score()>=armors[lightBars[j].Armor_index-1].Score()))armors[lightBars[j].Armor_index-1] = armor;
